...
static CGFloat kHHDoubleNumber = 2.0f;
static CGFloat kHHHalfNumber = 0.5f;
static CGFloat kHHMultiple = 1.4f;
@interface HHAnimatePageControl()<UIScrollViewDelegate>
...
@end
@implementation HHAnimatePageControl
...
- (void)prepareShow{
    ...
    if (_pageStyle == HHScaleColorPageStyle) {
        [self configDefaultIndicator];
    }
    else if (_pageStyle == HHDepthColorPageStyle){
        [self configDefaultIndicator];
    ...
    else if(_pageStyle == HHSquirmPageStyle){
    ...
    else if (_pageStyle == HHFillColorPageStyle){
        [self configDefaultIndicator];
    }
}
...
- (void)setPageStyle:(PageStyle)pageStyle{
    ...
    if (_pageStyle == HHSquirmPageStyle) {
...
- (void)setIndicatorDiameter:(CGFloat)indicatorDiameter{
    ...
    self.radius = _indicatorDiameter * kHHHalfNumber;
}

- (void)addIndicatorsWithIndex:(NSInteger)index{
    for (NSInteger number = index; number < _numberOfPages; number ++ ) {
        UIView *indicator = nil;
        if (_pageStyle == HHFillColorPageStyle) {
            indicator = [[HHIndiacatorView alloc] init];
            [(HHIndiacatorView *)indicator backView].backgroundColor = _currentPageIndicatorColor;
            [(HHIndiacatorView *)indicator frontView].backgroundColor = _pageIndicatorColor;
            [(HHIndiacatorView *)indicator backView].layer.cornerRadius = _radius;
            [(HHIndiacatorView *)indicator frontView].layer.cornerRadius = _radius;
            [self configZeroScaleAnimation:[(HHIndiacatorView *)indicator backView]];
            [self configZeroScaleAnimation:[(HHIndiacatorView *)indicator frontView]];
        }
        else{
...
- (void)setNumberOfPages:(NSInteger)numberOfPages{
    ...
    if (difference && self.superview) {
        ...
        if (_pageStyle == HHDepthColorPageStyle) {
...
- (void)configCurrentIndicator{
    ...
    if ([view isKindOfClass:[HHIndiacatorView class]]) {
        [(HHIndiacatorView *)view frontView].layer.timeOffset = 1.0f;
    }
    else{
        view.layer.timeOffset = 0.0f;
    }
...
- (void)configDepthView{
    ...
    for (NSInteger index = 1; index <= _currentPage; index ++) {
        ...
        con.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber) + (index - 1) * (_indicatorDiameter * _indicatorMultiple + _indicatorMargin);
    }
}
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            if (_pageStyle == HHScaleColorPageStyle) {
                ...
                if (currentIndex) {
                    ...
                    currentCon.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber);
                    lastCon.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber) + currentIndex * (_indicatorDiameter * _indicatorMultiple + _indicatorMargin);
                }
            }
            else if (_pageStyle == HHFillColorPageStyle){
                [(HHIndiacatorView *)currentIndicator frontView].layer.timeOffset = 1.0f;
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if (rate >= 0.0f && rate <= _numberOfPages - 1) {
        ...
        if (_pageStyle == HHScaleColorPageStyle) {
            ...
            if (timeOffset - kHHHalfNumber <= 0.0f) {
                halfTimeOffset = timeOffset * kHHDoubleNumber;
            }
            else{
                halfTimeOffset = (CGFloat)ABS(timeOffset - 1.0f) * kHHDoubleNumber;
            }
            ...
            if (isNoAnimationScroll) {
                ...
                currentCon.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber) + (currentIndex - (lastIndex > currentIndex ? -1 : 1)) * (_indicatorDiameter * _indicatorMultiple + _indicatorMargin);
            }
            else{
                currentCon.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber) + (currentIndex - timeOffset) * (_indicatorDiameter * _indicatorMultiple + _indicatorMargin);
            }
            lastCon.constant = (_indicatorDiameter * _indicatorMultiple * kHHHalfNumber) + (timeOffset + (currentIndex ? : 1 ) - 1) * (_indicatorDiameter * _indicatorMultiple + _indicatorMargin);
        }
        else if (_pageStyle == HHSquirmPageStyle){
            
            if (timeOffset - kHHHalfNumber <= 0.0f) {
                timeOffset = timeOffset * kHHDoubleNumber;
            }
            else{
                timeOffset = (CGFloat)ABS(timeOffset - 1.0f) * kHHDoubleNumber;
            }
            CGFloat number = (_indicatorMargin - _indicatorDiameter) * kHHHalfNumber;
            self.squirmCenterCon.constant = rate  * kHHDoubleNumber * (_indicatorMargin - number) + _indicatorDiameter * kHHHalfNumber;
            ...
            [(HHIndiacatorView *)currentPointView frontView].layer.timeOffset = timeOffset;
            [(HHIndiacatorView *)lastPointView frontView].layer.timeOffset = 1.0f - timeOffset;
        }
        self.currentPage = currentIndex;
    }
}
...
- (void)layoutSquirmView{
    ...
    self.squirmCenterCon = [NSLayoutConstraint constraintWithItem:self.squirmView attribute:NSLayoutAttributeCenterX relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeLeading multiplier:1.0f constant:-_indicatorDiameter * kHHHalfNumber];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    CGFloat currentX = (_indicatorMultiple * _indicatorDiameter + _indicatorMargin) * _currentPage + _indicatorMultiple * _indicatorDiameter * kHHHalfNumber;
    if (point.x > currentX) {
        self.currentPage++;
    }
    else{
